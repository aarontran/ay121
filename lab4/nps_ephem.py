...
def main():
    ...
    """
    obs = obs_berkeley()  # TODO change to Leuschner
    obs_day = '2014/04/21 20:00'  # PDT
    init_time = ephem.Date(ephem.Date(obs_day) + 7*ephem.hour)
    print'\nEphemerides after (PDT): %s' % pdt(init_time)

    # North polar/galactic spur coordinates
    # (l,b) are galactic longitude, latitude
    # longitude l = 210 to 360, 0 to 20
    # latitude b = 0 to 90
    # So, covering a huge quadrant!  1/4 of available sky...
    sw_corner = ephem.Equatorial(ephem.Galactic(210, 0, epoch = ephem.J2000))
    se_corner = ephem.Equatorial(ephem.Galactic(20, 0, epoch = ephem.J2000))
    n_corner = ephem.Equatorial(ephem.Galactic(0, 90, epoch = ephem.J2000))

    sw_corner = catalog.make_obj('sw corner', sw_corner.ra, sw_corner.dec)
    se_corner = catalog.make_obj('se corner', se_corner.ra, se_corner.dec)
    n_corner = catalog.make_obj('n corner', n_corner.ra, n_corner.dec)

    corners = [sw_corner, se_corner, n_corner]
    ...
    plot_next_ephemeris(obs, corners[0], init_time)
    plot_next_ephemeris(obs, corners[1], init_time)
    plot_next_ephemeris(obs, corners[2], init_time)
    plt.show()
