...
def linf_lsq(x, y, funcs):
    """Returns coefficients [a_0, a_1, ..., a_n, b] such that
    y = a_0*f_0(x) + a_1*f_1(x) + ... + a_n*f_n(x) + b
    is a least squares best fit for data values x, y

    Wrapper method for lsq_solve

    Output:
        (see lsq_solve output)
        a: coefficients, 1-D np.array of length len(funcs) (num. columns)
        s2: sum of squared residuals
        a_vars: variances in coefficients
    """
    cols = tuple([f(x) for f in funcs])
    x_mat = np.vstack(col).T
    return lsq_solve(x_mat, y)
